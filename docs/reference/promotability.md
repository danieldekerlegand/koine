...
6. **KCB — the most work, and the most implemented.** **Five** independent counts as of 0.5.0. It had
   **no fold outstanding** — V-1…V-7 landed at 0.5.0 on 2026-08-26, as MA-6/MA-8/MA-9 did at 0.4.9 —
   ...
   the two newest counts — §4.2 backpressure and §4.3 autonomy posture — which **no longer lack a KCS
   encoding**: DR-12 and DR-13 were closed downstream on 2026-08-26 and confirmed by running the gate
   on 2026-09-03, so both are open on their **re-runs** alone. Count (ii)'s encoding exists but
   must be **extended** past the fold it predates (**DR-7**), which is the one artefact objection KCB
   still carries; §4.3 additionally carries
