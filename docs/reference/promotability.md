...
6. **KCB — the most work, the most implemented, and now the most walked.** **Five** independent
...
- **Rows 5 and 6 are now coupled, and that is new.** KMI's remainder **strictly contains** KCB count
  ...
  waits on them — so KMI cannot promote before KCB has cleared one of its five. KCB stays last only
  because its remainder contains that *plus* four more counts, a clause and W3. The honest reading is
  that these two rows should be read together: **any promotion argument for KMI is partly a promotion
  argument for KCB's count (i)**.
...
**The seven things that stand between KCB and `ratified`, and who owns each.** KCB's row is the
longest on the page, so the register is pulled out flat. Nothing here is scheduled; *unowned* means
unowned by any tasklist in any repo, and it is recorded that way deliberately rather than parked
against a tasklist that would make it look otherwise.
...
| 6 | **DR-7** — `kcs:live-schema-mutation` *exists* but **predates** the fold, so it must be **extended** to the scenario's F1–F13 set (ten of thirteen need declared console extensions) before a re-run asserts anything about §7 | downstream, under [ADR-0001](../../decisions/ADR-0001-control-plane-topology.md) — count (ii) | not a koine edit | **unowned** |
...
Then all five counts are re-run. Items 1–5 are additive and KCB-only; **items 6 and 7 are not koine
work at all**, which is the reason a reader who watches only this repo will see KCB stop moving before
