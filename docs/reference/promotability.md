...
5. **KINP — the real work is done, and it moved KINP up this list in substance if not in rank.**
   ...
   **That re-run has now been walked, on 2026-09-03, and it is clean** — Step 1 holds and Steps
   2/3/4/6 flip, by hand against the prose rather than by replaying `kcs:multi-authority` (**DR-8**).
   **KINP's koine-side work is finished.** What that leaves is the correction of this paragraph's
   own earlier claim: *"a single re-run and nothing else"* was wrong, and the KINP row above always
   said so — the **KCS-encoding** condition binds on top of the prose leg, and `kcs:multi-authority`
   asserts none of the folded clauses, so it must be **extended** (DR-8, the DR-7 shape). KINP is
   therefore no longer the cheapest *koine-side* promotion by amount of koine work — it has none
   left — but it is not promotable either, and its remaining blocker now sits in the same place as
   KFT's and KCB's: **downstream, under [ADR-0001](../../decisions/ADR-0001-control-plane-topology.md),
   and unowned.**
