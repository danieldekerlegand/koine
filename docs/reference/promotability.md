...
has taken the work, and this repo's `tasks/chief/` holds a **queued** tasklist naming item (5)
(`970-fold-bp-7-live-adjustment`). *Queued* is not *merged* and it has not run, so no count moves and
no cell above is marked met — but a reader should not take it for un-scheduled. Three tasklists
**have** run, and all three left their item in the same place: `940-fold-v-9-and-v-11` folded V-9 and V-11 at KCB 0.5.3 and re-ran count (ii) the
...
**(i)** the same day, which is why item (2) now names **MT-2 and MT-3**. **No count moved for any of
the three**, and all three items are again **unowned** — which is the pattern
