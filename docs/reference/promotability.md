...
| 5 | KMI | **1 fold, and it is KCB's** (MA-17 + MA-18, one §4.5(a) edit) + 1 re-run — **MA-13 folded 2026-09-12 as KMI 0.3.8**, leaving KMI no koine-side item of its own | DR-4, DR-8/DR-7 · **plus all of KCB count (i)** | Cost went **up** twice, then **sideways** on 2026-09-12 (MA-12 and MA-13 folded, MA-17/MA-18 in their place) |
...
- **Rows 5 and 6 are now coupled, and that is new.** KMI's remainder **strictly contains** KCB count
  ...
  (`chief/930`, KMI 0.3.8), so KMI's last koine-side item of its own is gone** and the coupling is now
  total: every remaining item on row 5 is either KCB's or downstream.
...
day and also did not close (MA-17, MA-18). All of KCB's counts and both of KMI's have now been re-run;
**none closed**, so the
...
has taken the work, and this repo's `tasks/chief/` now holds **queued** tasklists naming items (2)
(`960-fold-mt-1-version-per-plan-leg`), (3) (`940-fold-v-9-and-v-11`), (4) (`950-fold-v-10-bp-8-ap-9`)
and (5) (`970-fold-bp-7-live-adjustment`). *Queued* is not *merged* and none of the four has run, so
no count moves and no cell above is marked met — but a reader should not take those four for
un-scheduled. Items (1) and (8) have no tasklist, and (6) and (7) are not koine work at all.
