...
6. **KCB — the most work, the most implemented, and now the most walked.** **Six** independent
   ...
   **V-10** stands unfolded and would have held it open on a clean walk of the three steps.)*
...
(`960-fold-mt-1-version-per-plan-leg`), (4) (`950-fold-v-10-bp-8-ap-9`) and (5)
(`970-fold-bp-7-live-adjustment`). *Queued* is not *merged* and none of the three has run, so no count
moves and no cell above is marked met — but a reader should not take those three for un-scheduled.
Item (3)'s tasklist (`940-fold-v-9-and-v-11`) **has** run: V-9 and V-11 are folded at KCB 0.5.3 and
count (ii) was re-run the same day, which is why item (3) above now names the **four findings that
re-run returned** rather than the two it folded. **Count (ii) did not move**, and the work is again
**unowned**. Items (1) and (8) have no tasklist, and (6) and (7) are not koine work at all.
