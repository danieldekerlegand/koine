...
2. **KCS — the read-and-confirm pass was done, and it read *not clean*.** This row used to say the
   ...
   **meaning** missing here, and a runner that added the name tomorrow would still have to invent the
   basis.
   **KCS 0.4.0 landed on 2026-09-12 and this item does not move** (`chief/990`). Two new normative
   sections — **§9**, the exchange record, and **§5.1**, the evidence-precedence rule that could not
   stay informative — and neither touches the count. The reason is worth stating precisely, because
   §5.1 is *about* the same failure mode as R: two conformant runners, one run, two reports. R is an
   open **comparison** basis under `structure_matches`; §5.1 fixes an open **evidence** basis under
   three other predicates. They are the same class of defect on two different axes, and closing one
   supplies nothing to the other — a runner handed §5.1 tomorrow would still have to invent R's
   basis. What 0.4.0 *does* change on this row is the cost of the open question behind it: §7
   question 1 asks whether a fixed core needs a conformance obligation on the **runner**, and §5.1
   is fixed-core surface a **name** check cannot see at all, since the evidence rule appears nowhere
   in a scenario document. So the row's work is unchanged (fold R, then re-validate) and the
   question upstream of it got harder rather than easier.
   **The ordering below is unchanged, and that is a claim worth checking rather than assuming.** KCS's
   ...
   two re-runs plus a missing artefact and six sections read cold, which is more, not less. (Three
   re-runs and five folds since 2026-09-12 — see the third count in item 3.)
3. **KFT — three re-runs, one missing artefact, and more cold reading than it looks.** (*"Two
   re-runs"* until 2026-09-12; the third arrived with the audio leg at the end of this item.) One re-run is
   ...
   **A third count landed on 2026-09-12, and it is the one nobody was counting.** 0.7.0's two audio
   `modality` rows had been recorded as *unexercised vocabulary* since 2026-08-27 — and explicitly
   *"not a third gate"*, on the reasoning that a token in a closed vocabulary cannot break a manifest
   that does not name it. That reasoning was sound and the conclusion was provisional: the leg that
   exercises them, [`../../scenarios/kft-audio-modalities.md`](../../scenarios/kft-audio-modalities.md)
   (`chief/980-audio-modality-pickup`), walks one job per row and returned **AU-1…AU-6** with **AU-1
   blocking** — §3.1 mints `audio-to-audio` on the property that its corpus is *paired asset↔asset
   with no caption side*, and §4.1's FT-I bullet, the only pairing carrier KFT names, types a row as
   *"a KMI `asset` id **and its text**"*, so a published row has **no expressible supervision**. The
   other five are scope and carrier breaks in §3.1's own additivity sentence (AU-2), §2/§8.1's
   port-based discrimination (AU-3 — the audio pair is the **only** duplicate in §3.1's *Data-plane
   port* column), §7's cardinality (AU-4), §3.3.2's conversion matrix (AU-5) and §5.3's export matrix
   (AU-6). **Zero model breaks** — §4.2's aggregate, §4.3's union, §5.4's inheritance, FT-J and
   §8.1's grades were each attacked on an audio corpus and held — so all six are additive, **KFT-only**
   and **unowned**, and counts (i) and (ii) are restated and neither moves. So KFT's koine-side work
   is **five folds in one spec** across **three** counts (FT-W + FT-X in one §4.1/§4.2/§4.3 edit,
   FT-Y in §5.2/§6, AU-1 + AU-4 in one §4.1/§7 edit, AU-2 a sentence, AU-3 in §2/§8.1, AU-5 a matrix
...
5. **KMI — the re-run happened twice, was not clean either time, and the count grew two folds.**
   ...
   *work* moved again onto the other plane: KMI's remaining koine-side item is **MA-13 alone**
   ...
   MA-17 + MA-18, count (ii)'s MT-1 fold, and count (ii)'s re-run — is **KCB's**. *(Updated
   2026-09-12: MT-1 is folded at KCB 0.5.6 / 0.5.7 and does not reproduce; what KMI's count (ii) now
   waits on is **MT-2 + MT-3**, still KCB's.)* The half-KMI
   ...
   **MA-13 was then folded on 2026-09-12 (`chief/930`) as KMI 0.3.8** — §7.1(d) names the **serving**
   ...
   on is KCB's (MA-17 + MA-18, MT-1 — folded at 0.5.6 / 0.5.7 the same day and replaced by **MT-2 +
   MT-3** — and count (ii)'s re-run) or downstream (DR-4, DR-8/DR-7).
...
6. **KCB — the most work, the most implemented, and now the most walked.** **Six** independent
   ...
   **V-10** stands unfolded and would have held it open on a clean walk of the three steps. Updated
   ...
   too little*.)*
   *(Updated again the same day: **MT-1 is folded** at KCB **0.5.6** (§3's path leg names the version
   it was matched over) and **0.5.7** (§4.4c refuses a resolved major that differs from a presented
   plan leg), count (i)'s same-day re-run finds that **it does not reproduce**, and F/G/J/K/L hold for
   a second consecutive walk — so that edit leaves the pile and two take its place, **MT-2** in §3 and
   **MT-3** in §4.4c. MT-2 takes the axis tally to **six** on the register and is the **seventh**
   finding on it overall: §3's fold claims *"it mints no field"*, which is true of a **leg** and false
   of a **path** — no clause of §3, §4 or §7 types a path request or a path result, and §4.4c types
   `planned_leg` *"as §3 returned it"* against that absence, so an unparseable plan is an **absent**
   plan. MT-3 is a **scope** break: the cross-check binds the party that cannot detect the condition,
   and no response names the resolved major.)*
   *(Updated once more the same day: **BP-7 is folded** at KCB **0.5.8** — §4.2b owes a live adjustment
   on §4.2d's channel one of `applied` / `cannot-honour` / `unsupported`, with *silence is not one of
   them*, the registration-time rule extended past the moment it was scoped to in §4.2c's own shape —
   and count (iv)'s same-day re-run finds that **it does not reproduce**, with blocking **BP-5** holding
   for a third consecutive walk and Step 8's four holds surviving re-probing. So that edit leaves the
   pile and two take its place, both in §4.2b/§4.2d and both one edit: **BP-9** (High) and **BP-10**
   (Med-High). **BP-9 takes the axis tally to seven on the register and is the eighth finding on it
   overall**, and it is the sharpest instance yet, because the rule it breaks is one this repo folded
   itself and this fold itself widened: §4.2b names three outcome tokens, requires an echoed
   subscriber-minted **id**, and fixes **no frame name, field, key or envelope** for the request, the
   answer or the id — while §4.2d's *an unnamed frame is one every conformant participant MUST ignore*
   rule, quoted from §7.3g inside the very bullet 0.5.8 widened to run in **both** directions, makes
   exactly that frame discardable. The MUST therefore binds only a producer that already understood the
   frame, two conformant implementations discard each other's, and (b)'s fail-closed reading makes the
   failure **silent**; it reproduces with **one producer and one subscriber**, and §4.2e's
   approaching-the-ceiling MUST has been routed at the same absent carrier since 0.4.7. BP-10 is
   **MA-15's and MA-19's rarer shape** — a collision between two clauses of one fold: the answer is owed
   per **frame** and two of the three ways are written per **operand**, and `applied` does not fix which
   set it enumerates.)*
...
| 3 | KFT | **5 folds, one spec** (FT-W + FT-X in one §4.1/§4.2/§4.3 edit; FT-Y in §5.2/§6; **AU-1 + AU-4** in one §4.1/§7 edit; **AU-2** a sentence; **AU-3** in §2/§8.1; **AU-5** a §3.3.2 disposition; **AU-6** a §5.3 row) + **3** re-runs + the reopened pin precondition | **DR-14** — the audio leg has no encoding and no run (2026-09-12); its other two encodings exist and ran | Cost went **up** three times |
...
- **The downstream run is now citable — with two conditions.** Under
  ...
  [`../../scenarios/README.md`](../../scenarios/README.md#findings-from-the-run-dr-1dr-14): `green`
...
**And a fourth thing nobody counted, which arrived on 2026-09-12.** The entry above says *"exactly
three conditions"*, and that was the honest reading of KFT's own *Pressure test* section, which
recorded 0.7.0's two audio `modality` rows as **unexercised vocabulary** and explicitly *"not a
third gate"*. What made that provisional was written in the same paragraph: neither row *"may be
cited in a re-ratification until"* a pass walks one. A pass now has —
[`../../scenarios/kft-audio-modalities.md`](../../scenarios/kft-audio-modalities.md) — and it is
**not clean**: **AU-1…AU-6**, blocking **AU-1**, so **unexercised surface became a count the moment
it was exercised**, which is the general lesson and not an audio one. §3.3 and §8.1 are the other two
such surfaces on this spec, and AU-5 is a finding *in* §3.3. The shape of the findings is the one
this page has recorded on five specs now: **zero** model breaks (§4.2's aggregate, §4.3's union,
§5.4's inheritance, FT-J and §8.1's grades were each attacked on an audio corpus and held) and six
**carrier or scope** breaks, four of them sentences the spec writes about itself. KFT's koine-side
work is therefore **five folds in one spec** across **three** counts, and its artefact axis is no
longer clean either: **DR-14**, a thirteenth scenario with no encoding — recorded on arrival this
time, which is the only thing about that class of finding this repo has improved.
...
| 2 | **MT-2 + MT-3 — two findings, two edits**, and **MT-1 is done**: folded at KCB **0.5.6** (§3's NORMATIVE bullet — each leg of a returned path names the `(name, version)` it was matched over, and the projected cost is the cost of exactly those legs) and **0.5.7** (§4.4c's OPTIONAL `planned_leg` and its MUST — a resolved major differing from the presented leg's major is refused *plan mismatch*, naming **both**), and **it does not reproduce** on the re-run the same day, with F/G/J/K/L holding for a second consecutive walk. What that re-run returned in its place is the fold's own perimeter: **MT-2** (High, carrier) — §3 claims *"it mints no field"*, which is true of a **leg** and false of a **path**, since §3's *Query* bullet types `find` by four operand kinds returning *"matching manifests, ranked"* and §4's verb table types `discover` the same way, neither taking a (start port, goal port) pair or returning a path, while §4.4c types `planned_leg` *"as §3 returned it"* against that absence — so a provider compares a value whose shape is fixed nowhere, an unparseable plan is an **absent** plan, and MT-1's silence returns through the shape; it reproduces with **one registry and one provider**, and the fix is MA-8's shape applied to the path result. **MT-3** (Med-High, scope) — `planned_leg` is OPTIONAL by design (§4.4e), so the refusal binds the party that cannot know a selection happened, while a provider resolving at §4.4c(2) **selects among published majors** where (3) forbids exactly that, and **no response names the resolved major**; the fix is that a provider resolving at (2) while publishing more than one major names the resolved `(name, version)`, which is §4.5's move on the other verb. | KCB §3 (and §4's `discover` row) / §4.4c — count (i) | koine, additive | **unowned** |
...
| 5 | **BP-9 + BP-10 — two findings, one edit**, and **BP-7 is done**: folded at KCB **0.5.8** (2026-09-12) — §4.2b now owes a live adjustment on §4.2d's channel one of `applied` / `cannot-honour` / `unsupported` and *silence is not one of them*, the registration-time rule extended past the moment it was scoped to in §4.2c's own shape — and **it does not reproduce** on the re-run the same day, with blocking **BP-5** holding for a third consecutive walk. What that re-run returned in its place is the fold's own perimeter: **BP-9** (High, carrier) — three named outcome tokens, the request they answer and the subscriber-minted **id** that correlates them have **no frame name, field, key or envelope**, on a channel whose §7.3g rule (*an unnamed frame is one every conformant participant MUST ignore*) this very fold **widened to run in both directions** — so the MUST binds only a producer that already understood the frame, two conformant implementations discard each other's, and (b)'s fail-closed reading makes the failure **silent**; it reproduces with **one producer and one subscriber**, and §4.2e's approaching-the-ceiling MUST has been routed at the same absent carrier since 0.4.7 (three MUSTs, zero named frames, against §7.3g's four). **BP-10** (Med-High, collision) — the answer is owed *"in exactly one of three ways"* per **frame** while (ii) and (iii) are written per **operand**, so a multi-operand adjustment with one meetable and one unmeetable operand has no correct answer and the fail-closed default does not reach an **answered** frame; and `applied`'s *"operands now in force"* does not fix whether it enumerates the adjustment's operands or the subscription's whole effective set. The fix is one §4.2b/§4.2d edit — name the frames, their fields and the id's slot where §7.3g's own argument says the channel's vocabulary belongs, and settle atomicity in the same bullet set | KCB §4.2b/§4.2d — count (iv) | koine, additive | **unowned** |
...
has taken the work. **Four** tasklists have now run, and all four left their item in exactly the same
place: `940-fold-v-9-and-v-11` folded V-9 and V-11 at KCB 0.5.3 and re-ran count (ii) the
...
AP-10**; and `960-fold-mt-1-version-per-plan-leg` folded MT-1 at KCB 0.5.6 / 0.5.7 and re-ran count
**(i)** the same day, which is why item (2) now names **MT-2 and MT-3**; and
`970-fold-bp-7-live-adjustment` folded BP-7 at KCB 0.5.8 and re-ran count **(iv)** the same day,
which is why item (5) now names **BP-9 and BP-10**. **No count moved for any of the four**, and all
four items are again **unowned** — which is the pattern
this register exists to make visible: a fold that lands cleanly and a count that closes are different
events, and so far only the first has happened. Items (1) and (8) have no tasklist, and (6) and (7) are not koine work at all. **And closing any one of them promotes nothing**: each count gates one section, and a spec is promoted when **every** count on it closes — count (iv) gates §4.2 alone, so even a clean re-run of it would have left KCB exactly as unpromotable as it is, with the other five counts standing.
