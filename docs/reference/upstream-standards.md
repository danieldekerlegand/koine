...
below. The **W3C Trace Context** row was filled by `chief/990-kcs-emitted-telemetry-extension` (the
KCS emitted-telemetry extension), whose dated prior-art sweep is
[`kcs-telemetry-prior-art.md`](kcs-telemetry-prior-art.md). Every row, whoever filled it, is
reviewed on the one cadence.
...
| **W3C Trace Context** | **Level 1 — W3C Recommendation 2021-11-23** (the document states it *“includes editorial updates since the 6 February 2020 W3C Recommendation”*) | KCS §9 (exchange-record correlation) | ✅ | **Profiled by reference for one axis only: correlation.** Where a KCS §9 exchange record carries a correlation identifier it is a Trace Context `trace-id`, not a koine-minted one — the **Hugging Face `base_model`** reasoning in this table, applied to a deployed convention that already rides the HTTP carrying MCP and A2A. What koine reads is `traceparent`'s `trace-id` / `parent-id`; `tracestate` is vendor space and koine claims none of it. The standard constrains **propagation** and says nothing about what a record carries, so the pin's blast radius is exactly one field. **Live drift, deliberately not taken:** **Level 2** is a **Candidate Recommendation Draft of 2024-03-28** adding the random-trace-id flag and id-generation guidance; koine stays on the **Recommendation** for the [OWL-Time](#the-pin-table) row's reason — a CR Draft is revisable at any time by its own boilerplate and the two fields koine reads are unchanged between the levels, so moving would buy nothing. Re-check when Level 2 reaches Recommendation. **The row was entered ⬜ and moved to ✅ on 2026-09-12**, the day the citing clause landed: the sweep that chose this disposition ([`kcs-telemetry-prior-art.md`](kcs-telemetry-prior-art.md)) ran before the clause was written, and a row claiming a spec pins something no spec cites is the drift this table exists to surface. The citation is **KCS 0.4.0 §9(b)/(e)** — the `trace` field carries Trace Context `trace-id`/`parent-id` **as propagated**, and §9(e) states that koine mints no parallel trace identifier. **OpenTelemetry takes no row**, and that is the other half of the same verdict — KCS *resembles* its trace data model and delegates to no clause of it, so it is prior art recorded in the sweep, and a prior-art link is not a pin. |
...
|---|---|---|---|
| **2026-09-12** | **one — the new W3C Trace Context row only**, fetched fresh (Level 1 and the Level 2 CR Draft), alongside OpenTelemetry, its semantic conventions and CloudEvents, which are read in [`kcs-telemetry-prior-art.md`](kcs-telemetry-prior-art.md) and take no row. **No existing row was re-fetched**, so this is a row-addition sweep and not a table sweep: the header's *Last reviewed* date and the quarterly floor below are deliberately left where they stood. | *none* | `chief/990-kcs-emitted-telemetry-extension` |
