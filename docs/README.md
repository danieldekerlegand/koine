...
- [The `eval` relation domain — the proposal as received, and the verdict](reference/eval-relation-domain.md) — a proposal koine had never received in writing, recorded and then decided: the two relations as asked, the governance the verdict is measured against, the redundancy test against every published relation, why the two are not redundant with *each other* (the derivation has no form — `prov` is not a relation argument), and the no-score boundary upheld with the trigger that would reconsider it
- [The emitted-telemetry prior art — what was read on 2026-09-12, and the verdict](reference/kcs-telemetry-prior-art.md) — the dated sweep that gates KCS's exchange-record clause, read against the eleven things the record carries rather than in the abstract: W3C Trace Context **profiled by reference** for correlation alone (Level 1, with the Level 2 CR Draft deliberately not taken), OpenTelemetry **resembled and not adopted** — three of the eleven fields covered, and its sampling model making an emitted record droppable by construction, which is why emission stays optional and an absent record asserts nothing — and CloudEvents read and dismissed with its re-open trigger stated
