...
- [Documentation drift — what was read against the tree, and what was wrong](reference/doc-drift-corrections.md) — the register of record for the documentation sweep: six claims that parsed, linked and resolved perfectly while describing a tree that had moved, each with what it said, what the tree says, and the date it stopped being true; the shape five of the six share (a restated fact whose home is elsewhere) and why four of the fixes **remove** the copy rather than refresh it; plus what the sweep deliberately left alone, and what its method cannot claim
