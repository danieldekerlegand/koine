...
`draft → candidate → ratified`. These eight documents are the executable-in-prose form of that
...
| [`kmi-otio-roundtrip.md`](kmi-otio-roundtrip.md) | KMI §4.2a / §9.5 | **Additive metadata survival** — a third-party OTIO round-trip drops the KINP asset-id carrier and leaves only stale or local paths, forcing an answer on safe id re-attachment. | M-1 | ⬜ **focused pressure leg** — follow-up to `e2e-media-transform` |
...
encodes the eight documents below, `agora chief/63-run-kcs-over-live-links` runs the suite over real
...
seven other scenarios *in* it, and every place the format could not say what a scenario needed. That is
