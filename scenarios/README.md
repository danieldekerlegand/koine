...
extended — **DR-8**, the **DR-7** shape) and **neither KCB nor KMI is promotable**. **KCB's steps were
then walked a third time on 2026-09-12**, against the version that writes ADR-0014's clause (**KCB
0.5.1**): the 2026-09-03 blocker **does not reproduce**, Step 6 flips and Step 7 half-flips, but
**Step 5 still does not flip** → **MA-14**, **MA-15** and **MA-16**, three carrier-or-scope breaks in
the new clause (two of them present in the ADR's Decision verbatim), all additive, all KCB-only and
**one edit** between them. **Count (iii) still does not close.** **KMI's steps were walked twice more on the
same day**: first against **KCB 0.5.2 / KMI 0.3.7**, the versions that fold **MA-12** — which **does
not reproduce**, while §4.5's *perimeter* breaks on **MA-17** (High, carrier: the four `fetch`
outcomes are named and no field is fixed to read them from, on the one verb KCB types by no protocol
— so the implementation-private status string KMI §7.1(f) forbids returns as the **slot**) and
**MA-18** (Med, mis-route), both **KCB's** — and then against **KMI 0.3.8**, which folds **MA-13**.
**MA-13 does not reproduce either**, and **Step 9 half-flips** on the perimeter of that fold's own
sentence: **MA-20** (High, carrier — §7.1(d) permits the `license`/`egress` pair to travel and fixes
**no shape, field, envelope or attribution** for it, so what arrives is a §2 field outside a §2
envelope) and **MA-19** (Med-High, collision — (e)'s per-axis reading composes a pair no participant
asserted and (d) carries it, against (d)'s own MUST NOT synthesize). **KMI count (i) does not close**
on either walk, and the pattern the two share is the one worth carrying: **a fold does not close its
own gate, and the re-run is where the fold's perimeter is found.** All five walks are recorded in the
scenario, one section each. Its *Fold status* section re-reads each finding against the folded specs, and
