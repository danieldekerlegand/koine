...
- `specs/capability-bus.md` — KCB 0.5.8, **candidate**. Control plane over MCP/A2A; cross-plane
  ...
  **eight** items since 2026-09-12, when count (vi)'s walk added (8) the **MA-17 + MA-18** fold
  ...
  owner on 2026-09-11; items (2), (3), (4) **and (5)** have each had a tasklist **run** (0.5.6/0.5.7,
  0.5.3, 0.5.4/0.5.5, 0.5.8) and each is **unowned again**, its fold having landed and its count having
  changed shape rather than closed: (1) was ADR-0014's four-part clause, a
  ...
  (iii)'s re-run returned against it, gating count (iii) alone; (2) is now the **MT-2 + MT-3** fold (§3 and §4.4c, two edits for two findings) that
  count (i)'s re-run returned against **MT-1**, folded at 0.5.6/0.5.7 and not reproducing; (3) **V-9 +
  V-11** (§7.1); (4) is now the **V-16 + AP-10** fold (§7.3g, one edit for two findings) that counts (ii) and (v) returned against **V-10 + BP-8 + AP-9**, folded at 0.5.4/0.5.5 and none of the three reproducing; (5) is now the **BP-9 + BP-10** fold (§4.2b/§4.2d, one edit for two findings) that count (iv)'s re-run returned against **BP-7**, folded at 0.5.8 and not reproducing; (6) **DR-7**, the encoding extension to the F1–F13 set — downstream under ADR-0001, and
  ...
  **0.5.4 and 0.5.5 (2026-09-12, two patches, one publication cycle) are the V-10 / BP-8 / AP-9 fold —
  three findings, three pressure legs, ONE hole**, the axis ADR-0014 named in advance and explicitly
  deferred *"to whichever pressure test breaks it"*. Three did. **0.5.4 is the bump half.** §7.2's
  normative table *fixes what a provider MAY change under a given bump*, and `volume` (§4.2a, 0.4.7)
  and `effect` (§4.3a, 0.4.8) arrived afterwards, each declaring a **minor** *"(§7.2)"* in its own
  section on which the whole of its visibility rests — *the version moves, so a pinned subscriber can
  see it* — against a table with **no row for either**, the nearest reaching them reading **patch**.
  The table gains a `volume` row and an `effect` row, both **minor**, both **agreeing with** what those
  sections already declared, so **no provider obligation changes** and what changes is that the
  declaration has an authority behind it; the removed disagreement is **recorded in §7.2** rather than
  quietly repaired. The merge-key consequence is followed through to **§3.1(d)**, where the absent rows
  actually bit: a field outside the merge key is rescued **not by the key but by the `version` its
  declared bump moves** (because `version` *is* in the key), so **four** of ADR-0014's five carriers —
  `cost`, `binding`, `volume`, `effect` — now fail the converse and are returned as the two entries they
  are, and **one** is not rescued and cannot be, the deprecation marking being applied to a published
  capability **in place**, which is the case §3.1(d)(i)/(ii) were written for. **0.5.5 is the signal
  half.** Three sections told a live subscriber it would be signalled on **§4.2d's** channel — §2.4 and
  §7.2's `binding` row (0.5.0), §4.3a for an `effect` class (0.4.8), §7.2's `volume` row (0.5.4) — and
  **§7.3g named three frames, none of which is any of them** (`successor_published` carries a
  *successor's* `binding`, never the bound one's). §4.2d's own *tolerate a producer that never sends
  one* is what makes that a gap rather than a licence: a frame nobody names is one every conformant
  subscriber may discard, so an **undefined** signal and an **absent** one are the same signal, and
  §7.3g's contrasting property — a producer emitting none is **detectable** because the frames are
  *named* and KCS §5 can assert their absence — was true of three facts and false of three others.
  §7.3g's table gains a **fourth** frame, **`entry_changed`** — one **generic** frame rather than three,
  the shape the break-test's own fix column proposed, and the shape that carries to the next operand
  minted outside the digest without re-editing §7.3g — announcing that a **declared non-shape operand**
  on the bound entry has moved (`binding`, `volume`, `effect`), carrying the capability's **new
  `version`** and naming which moved. It obeys §7.3g's existing rules unchanged: rides §4.2d's
  **existing** channel and mints no second mechanism, **precedes** the fact it announces (a producer
  moving a bound `binding` without a preceding frame is now non-conformant, parallel to `removal`), is
  ignored by a subscriber that does not understand it, and is **bounded** — MUST NOT announce a change
  of **shape** (a new major, carried by `successor_published`) and MUST NOT stand in for `deprecated`,
  which has its own frame and its own §2 carrier. A `cost` change **MAY** ride it and is left a MAY on
  purpose (§5 states no telling obligation; a re-price fails closed at the gate). §2.4's *"never left to
  a failed dial"* is **qualified rather than deleted** — met for the binding form the channel exists on,
  **not** met for a cached discovery binding, which is **DEFER-D**, unmoved with its trigger intact —
  and §2.4's *bounded on purpose* paragraph is byte-unchanged. Both are **patch**: every field and frame
  additive, no verb/plane/port kind/grant/authority role added, **§7.1 step 1's drop list
  byte-unchanged so no published `schema_id` or digest moves** (the frame is not a card field), and
  **0.6.0 stays spoken for** by §2.3's removal. The bump axis is named rather than assumed, because this
  repo has got it wrong twice: §7.2's table governs **a published capability's** bumps and decides
  nothing about KCB's own spec version, which moves on §7.3b's axis — the table is consulted for the two
  questions it *does* answer (no digest moves, no live subscriber breaks) and both are **No**.
  **Counts (ii), (iv) and (v) were then each re-run by hand the same day, and NONE of the three
  closes.** Three verdicts, one per count, against the section that count gates — never a combined one,
  which would destroy the structure the six counts exist to keep — and all three against the **prose**,
  since the encodings of all three legs are green and now **predate** this fold (**DR-7**'s shape, on
  three counts at once; `kcs:live-schema-mutation` now predates **four** publications of the spec it
  gates). **V-10, BP-8 and AP-9 all fail to reproduce**, each re-attacked on the ground it was filed on.
  **(ii)** (Steps 2, 3, 7, 10 walked; 5/6/9/11 restated unmoved on byte-unchanged clauses, so
  **V-12/V-13/V-14/V-15 stand as filed**) → **V-16** (Med-High, payload): the frame is normative in
  exactly two things — carry the new `version`, **name** which operand moved — and carries **no new
  value**, so §7.3g's *puts the new address in its hands* and §2.4's *dials the new address* follow only
  from a re-`describe` **no clause requires** and §7.2's pull-side invariant says is not owed, with the
  asymmetry visible **inside the table** (`successor_published` carries a `binding`; this frame does
  not) and a second, smaller instance in naming the operand but not the **port**, `volume` being
  port-level. **(v)** (Step 6) → **AP-10** (Med-High), the same defect on the `effect` axis and
  **sharper**: §4.3f evaluates a `subscribe` posture **once at registration** and points a live binding
  at this signal alone, so §4.3c's intersection has no class to intersect and §4.3d's floor has **no
  evaluation point** on a live stream — §7.2's *refusal at the next dispatch* being right for `invoke`
  and being exactly what a stream does not have — and the direction of failure is a **class narrowing**
  a conformant subscriber keeps delivering under. **§4.3 itself needs no change.** **(iv)** (Step 8,
  Step 1 re-checked) → **nothing**, and the decision is recorded rather than left as an omission: the
  same omission was put to `volume` and does **not** bite, because §4.2a's purpose for `volume` is
  discrimination *before* binding, §4.2b's subscriber-declared limits still bind on a live subscription,
  and a moved `volume.cost` fails closed at §5's ceiling under §4.2e's delivery-time evaluation. That
  leg's Step 8 also corrects its **own** wording: the hold *"§7.2's table is not disturbed"* is now
  literally false (two rows), and the property it was asserting — **no live subscriber breaks** — is
  what holds, both new rows reading *No*. So count (ii) reads *fold V-12 + V-13, V-14, V-15 and **V-16**
  (one §7.3g edit, with AP-10), then re-run Steps 5, 6, 9 and 10*; count (iv) is held open by **BP-7**
  **alone**; count (v) by **AP-10** and by ADR-0013's **W3**. V-16 and AP-10 are **one additive,
  KCB-only §7.3g edit** and **unowned**. The **axis tally stays at five** (MA-8, V-10, BP-7, ADR-0014's
  marking, MA-17) because V-16 and AP-10 are not *nothing carries it* but *the carrier carries too
  little* — they are the **fourth consecutive** fold in this repo to break on its own perimeter rather
  than its model, after §4.5's MA-17, KMI §7.1(d)'s MA-19/MA-20 and §7.1's V-12/V-14. Dispositions:
  `docs/reference/capability-versioning-fold-dispositions.md` § *V-10, BP-8 and AP-9 — three findings,
  three legs, one hole*. **No version and no clause moved for the three walks** — every normative clause
  of §1–§8 byte-unchanged, §7.3g's four rows and §7.2's table included, no published digest moves; the
  edit is three scenario sections, three gate paragraphs, a *Pressure test* paragraph and a changelog
  entry. **All six counts stay open and KCB is not promotable.**
  **0.5.6 and 0.5.7 (2026-09-12, two patches, one publication cycle) are the MT-1 fold — count (i)'s
  delta, and the oldest count's first fold.** 0.5.4/0.5.5's form is reused because MT-1 has a
  **carrier** half and an **enforcement** half. **0.5.6, the statement.** §7.1 makes `(name, version)`
  — *"not the name alone"* — the unit of discovery and §3's *Ranking across versions* rule makes the
  registry match the **highest satisfying version** first, so a composed path is **already built over
  a specific version of each leg**; §3's *Composition* bullet named ports, planes, providers and a
  projected cost, and **no version**, so a plan could not **disagree** with anything. §3 gains a
  NORMATIVE bullet: each leg of a returned path MUST name the `(name, version)` it was matched over —
  an exact version, never a range, never the name alone, §7.1's **`0.0.0`-unknown as a value** where
  the entry carries none — the **projected cost is the cost of exactly those legs**, read from the
  same entries, so a caller gating spend on the plan gates against the leg it planned over; and a plan
  leg is stated to be the registry's **account of what it matched**, reserving nothing, expiring by
  nothing, binding no provider (ADR-0001's addresses-not-proxy rule untouched), with which major an
  `invoke` reaches left to §4.4. **0.5.7, the refusal.** `invoke` MAY carry an OPTIONAL
  **`planned_leg`** and where the **resolved** major differs from the presented leg's major the
  provider MUST refuse **plan mismatch**, naming **both** — not a new convention but **the instrument
  this bus already shares**, applied a **third** time (§5's unstated `budget_units` unit at MA-6,
  §4.4c(3)'s want of a version, and now a plan disagreeing with what was resolved). Five rules bound
  it: the comparison is on the **major and only the major**, so `1.4.0` against major 1 is not a
  mismatch; a leg naming another capability is refused and **never ignored** (§7.2's
  ignore-unknown-**fields** rule is about a field a party does not understand, and here the field is
  understood and its *value* is the disagreement); `0.0.0`-unknown is compared like any other and
  **nothing matches leniently**; it is a **cross-check, never an operand of resolution** — applied
  after the order, never a fifth case, never a stand-in for a missing `version`, minting **no default
  and no *highest published* fallback**, so V-5's fail-open inversion is not reintroduced from the
  other side; and the **grant is untouched and is a different party** — a resolved major outside the
  granted major is still refused at the gate, before the work. The **rejected alternative is on the
  record**: making §4.4a's `version` operand REQUIRED of a caller fulfilling a path leg would make
  planning through the registry a **pinning obligation**, against §4.4e. Both **patch**, and the axis
  is named rather than assumed — §7.2's table governs *a published capability's* bumps and decides
  nothing about KCB's own spec version, which moves on §7.3b's axis; the table is consulted for the
  two questions it answers and both are **No**. Additive: both operands OPTIONAL both directions, an
  `invoke` carrying none of §4.4's operands against a one-major provider behaves exactly as at 0.4.9,
  **no field, verb, plane, port kind, grant or authority role added**, no ranking rule and no
  resolution case changes, §7.1 step 1 byte-unchanged so **no published `schema_id` or digest moves**,
  §7.2's table undisturbed, and **0.6.0 stays spoken for** by §2.3's removal — checked, not tripped.
  **No count closes and none is added**: MT-1 was found inside count (i)'s own walk, so that count
  changes shape rather than gaining a sibling, and the clause re-enters validation on **(i)**, not on
  §7.5's (ii) — two clauses of one section may gate on two different counts.
  **Count (i) was then re-run by hand the same day (2026-09-12) against 0.5.7 / KMI 0.3.8 — the first
  text carrying both halves — and it does NOT close.** Steps **1, 3, 4** and **8** walked against the
  **prose**, never a replay: that encoding now predates **eight** publications of the spec it gates
  (**DR-7**), is the one `kcs:kmi-otio-roundtrip` re-titles over a single fixture (**DR-4**), and
  answered Steps 3/4's provider half from a `standin` (**DR-1**). **MT-1 does not reproduce**,
  re-attacked on the ground it was filed on: the plan says what it planned over, the refusal names
  both majors, the major-only comparison holds under probe, the operand cannot be used to select, a
  leg naming another capability is refused rather than discarded, and the grant rule is byte-unchanged
  and correctly names a different party. **F, G, J, K and L hold for a second consecutive walk**, and
  **K is stronger by construction** — the quote is now read from the same entries the legs are named
  from, so delta K and delta F compose rather than merely coexist. Two new **perimeter** deltas, both
  on the fold published hours earlier: **MT-2** (High, carrier) — §3's *"it mints no field"* is true of
  a **leg** and **false of a path**, since §3's *Query* bullet types `find` by four operand kinds
  returning *"matching manifests, ranked"*, §4's verb table types `discover` the same way, **neither
  takes a (start port, goal port) pair or returns a path**, and §4.4c nonetheless types `planned_leg`
  *"as §3 returned it"* against that absence — so a provider must compare a value whose shape is fixed
  nowhere, an **unparseable plan is an absent plan**, and MT-1's silence returns through the shape; it
  reproduces with **one registry and one provider**, and the contrast is MA-8, §4.5 and ADR-0014's
  marking, all of which minted a carrier for exactly this reason. **MT-3** (Med-High, scope) —
  `planned_leg` is OPTIONAL by design (§4.4e), so the refusal binds the one party that **cannot know a
  selection happened**, while a provider resolving at (c)(2) **selects among published majors** where
  (c)(3) forbids exactly that, and **no response names the resolved major** (KMI's `produced_by` names
  the run activity, not the capability version), so the disagreement is undetectable after the fact as
  well as before it — MA-12's shape on the other verb. Count (i) now reads *fold **MT-2** (§3's path
  request and result, in MA-8's shape) and **MT-3** (§4.4c names a resolved major where it selected
  among several), then re-run Steps 1, 3, 4 and 8 again*; both additive, both KCB-only, both
  **unowned**, and the other five counts are restated and none moves. **MT-2 is the seventh finding on
  ADR-0014's axis** (after MA-8, V-10, BP-7, the marking, MA-17 and MA-20) and this is the **fifth
  consecutive** walk in this repo to break on a fold's perimeter rather than its model. **No version
  and no clause moved for the walk** — §1–§8 byte-unchanged, §3's MT-1 bullet and §4.4c's cross-check
  included, no published digest moves; the edit is two gate paragraphs, a *Pressure test* paragraph, a
  ...
  **That walk was run by hand the same day (2026-09-12), against 0.5.8 — the first text carrying BOTH
  of count (iv)'s folds — and count (iv) does NOT close.** One walk rather than two each re-reading the
  other's absence, which is what the gate paragraph asked for. Steps **3, 6, 7 and 8** walked; Steps
  **1, 2, 4 and 5** restated unmoved on the verified ground that §4.2a, §4.2c, §4.2e and §4.2f are
  byte-identical to 0.5.7. Prose, never a replay — `kcs:subscription-firehose` is green and now
  predates **three** publications of the section it gates (**DR-7**). **BP-7 does not reproduce**,
  re-attacked on the ground it was filed on: the three answers hold, the conformant-by-refusing rule
  holds, the fail-closed reading of silence holds, and §4.2g is untouched so the answer commits to no
  latency. **BP-8 does not reproduce**, **blocking BP-5 does not reproduce for a third consecutive
  walk** — §4.2d's host-not-on-the-path and one-channel-not-two bullets are byte-unchanged and the new
  obligation names exactly the two peers — and Step 8's four holds survive re-probing in the corrected
  *no live subscriber breaks* form. Two new **perimeter** deltas, both on the fold published hours
  earlier and both **KCB's own**: **BP-9** (High, carrier) — (b) requires an answer to *"a subscriber →
  producer adjustment frame (d)"*, names three outcome **tokens** and requires an echoed
  subscriber-minted **id**, and fixes **no frame name, field, key or envelope** for the request, the
  answer or the id, while §4.2d's *ignore what you do not understand* rule — **widened by this very
  fold to run in both directions** — makes an unnamed frame one every conformant participant MUST
  ignore. The criterion is **§7.3g's own**, folded at 0.5.5 to close **V-10** and quoted **inside the
  bullet 0.5.8 widened**, so the MUST binds only a producer that already understood the frame, two
  conformant implementations discard each other's, and (b)'s fail-closed reading makes the failure
  **silent** — MA-17's defect one plane over, and worse in one respect, since §4.5's outcomes at least
  sit in the table that types the verb while §4.2's frames are named in **no** table and §7.3g states
  there is *"one place its vocabulary is named"*. It reproduces with **one producer and one
  subscriber**, and §4.2e's approaching-the-ceiling MUST has been routed at the same absent carrier
  since 0.4.7: **three** MUSTs on §4.2's half of the channel, **zero** named frames, against §7.3g's
  four. And **BP-10** (Med-High, collision) — the answer is owed *"in exactly one of three ways"* per
  **frame** while (ii) and (iii) are written per **operand**, so a multi-operand adjustment with one
  meetable and one unmeetable operand has **no correct answer** and the fail-closed default does not
  reach it (that frame *was* answered); and `applied`'s *"operands now in force"* does not fix whether
  it enumerates the adjustment's operands or the subscription's whole effective set, which differ
  whenever a registration-time operand is not restated. MA-15's and MA-19's rarer shape, a **collision
  between two clauses of one fold**. Both are **one additive, KCB-only §4.2b/§4.2d edit** and
  **unowned**; count (iv) now reads *fold BP-9 and BP-10, then re-run Steps 1–8 again*, and the five
  other counts are restated and none moves. **BP-9 is the eighth finding in this repo on ADR-0014's
  axis** (after MA-8, V-10, BP-7, the marking, MA-17, MA-20 and MT-2) and this is the **sixth
  consecutive** walk here to break on a fold's perimeter rather than its model. One thing is
  deliberately **not** swept in, with the reason: §4.2c's `resume` answers have no named carrier
  either and BP-9 is not filed against them, because they ride the `subscribe` request/response
  exchange the verb itself pairs and directs, where (b)'s answers ride an **unpaired** in-band frame
  with nothing to correlate to — which is exactly why (b) had to mint an id and (c) did not. **No
  version and no clause moved for the walk** — §1–§8 byte-unchanged, §4.2a–g included, no published
  digest moves; the edit is §4.2's gate paragraph, a changelog entry, a scenario section and the
  BP-7 disposition in `docs/reference/capability-versioning-fold-dispositions.md`. **KCB is not
  promoted and is not promotable**, and closing count (iv) would promote nothing on its own.
- `specs/media-interchange.md` — KMI 0.3.8, **candidate**. Media data plane; asset envelope +
  ...
  **MA-17 + MA-18 stay KCB's** work on KCB's verb, count (ii) is KCB's and unmoved (MT-1 — folded at
  KCB 0.5.6/0.5.7 the same day, not reproducing, and replaced on that count by **MT-2 + MT-3**), the
