...
| Status | Milestone | Tasklist |
|---|---|---|
| ⬜ | **Re-ratify KCB 0.4.1** — **two** gates now (KCB pressure-test §): re-run the media-transform scenario against the 0.3.0 AgentCard-extension shape (`capabilities.extensions[]`, `https://w3id.org/koine/kcb/manifest/0.3`), **and** a *clean* §7.5 break-test. The second gate's scenario has landed and run (`scenarios/e2e-live-schema-mutation.md`) and is **not clean** — V-2/V-4/V-5/V-7 blocking — so re-ratification now waits on the KCB **0.5.0** fold (Phase F2) plus a re-run — **and**, like every row here, on the KCS encodings of both scenarios (F4 · `62`/`63`). Two further counts have accrued since: **§3.1** registry federation (KCB 0.4.6), gated by `scenarios/e2e-multi-authority.md`, and **§4.2** subscription backpressure (KCB 0.4.7, 2026-08-26, `chief/70`), gated by a re-run of `scenarios/kcb-subscription-firehose.md` against the folded text — **four** in all, each gating only its own section. The fourth **also fails the conformance gate today**: that leg is the eleventh scenario and has **no KCS encoding**, so §4.2's clauses have no runnable document citing them ([`docs/reference/kcs-encoding-gate-verification.md`](docs/reference/kcs-encoding-gate-verification.md) §6.2); that encoding is downstream work and **unowned** | re-run `scenarios/e2e-media-transform.md` + `scenarios/e2e-live-schema-mutation.md` + `scenarios/e2e-multi-authority.md` + `scenarios/kcb-subscription-firehose.md` · — |
...
*Depended on* **Phase F4** — every row needs the KCS encoding of the scenario it re-runs. That dependency is **discharged**: `agora chief/75` encoded all nine scenarios and `agora chief/76` ran them over live links, both merged 2026-08-19 ([`docs/reference/kcs-encoding-gate-verification.md`](docs/reference/kcs-encoding-gate-verification.md)). It is discharged **for the nine scenarios of that date**, and two later ones re-opened it narrowly: `scenarios/kft-resume-checkpoint.md` (**DR-11**) and `scenarios/kcb-subscription-firehose.md` (**DR-12**) have no encoding and no owner, so KFT's second gate and KCB's fourth are blocked by F4 as well as by their own re-runs. Every other row is blocked only by its own outstanding pass. KMI's *prose* half clears the moment KCB's re-run passes. The caveat that used to read the other way is **closed**: a downstream result becomes **citable** evidence only once it is recorded in the scenario it ran (`## Downstream results`), and as of 2026-08-26 all **eleven** scenarios carry a populated one, with the run's own breaks opened as findings **DR-1…DR-12** (`84-record-the-downstream-results`). Read those before citing the run — two green scenarios sit over blocking deltas (**DR-7**, **DR-8**), and **DR-3** says the fully-live pass does not touch KGP's outstanding fixture gate.
...
- **Nothing was recorded back into koine** — ✅ **closed 2026-08-26** by
  `84-record-the-downstream-results`. All **eleven** scenarios now carry a populated
  `## Downstream results` section (the two newest record that they were never encoded and never
  run), the run's breaks are opened as findings **DR-1…DR-12** indexed at
  [`scenarios/README.md`](scenarios/README.md#findings-from-the-run-dr-1dr-12), and the same pass
  flipped the stale `planned`-nine **KCS encoding** column in
  [`scenarios/README.md`](scenarios/README.md#the-kcs-encodings-nine-of-eleven-exist-and-the-other-two-are-the-ratification-tail).
