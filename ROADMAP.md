...
the current count. That count was **0 of 6** when this paragraph was written (2026-08-26) and is **1 of 6** since **KGP 0.5.2** was re-promoted on 2026-08-28 (Phase 1's KGP row; corrected 2026-09-11, when this paragraph still said 0 while the status line said 1); Phase 1 held the remaining gate for four of the six — KGP's is now closed —
...
| ✅ | **Memory substrate** — joins as a KCB provider (KINP + KCB) — **joined**: tessera derives and serves its manifest as `tessera:agent:tesserad` and registers on start, degrading gracefully when no registry is up. Built against **KCB 0.2.0**'s standalone `/.well-known/kcb-manifest.json`, the form KCB 0.5.0 has since removed (§7.3f) — keeping pace with later KCB revisions is tessera-side work and unscheduled there | tessera `chief/80-kcb-provider` *(merged `71bd0bb`)* |
...
walked by hand on 2026-09-03 — twice for KMI's steps. **KINP's gate flipped in full; KCB's and KMI's
did not.** What remains is **three** small additive folds the re-runs themselves found (**ADR-0014**'s
clause for KCB §2/§3, **MA-12**'s `fetch` carrier for KCB §4 + KMI §7.1(f), and **MA-13**'s
...
### Phase F6 — Per-spec deferred design questions — ✅ delivered, 4/4 — every leg run; the questions no leg forced stay open by design (scale: S–M each, koine)
