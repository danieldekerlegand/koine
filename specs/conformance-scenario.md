...
Steps run in declared order by default; a step may declare `after: [stepId…]` for explicit
concurrency (mirrors the real interleavings the pressure tests exercised).

## 4. Execution & observation model

1. **Discover** participants via the registry; **open direct** MCP/A2A links (no proxy).
2. **Run** steps, recording every request, response, and stream frame into an **observation
   log** (each entry stamped with participant, plane, KINP ids touched, transaction time).
...
- **Control (KCB):** `capability_path_exists(from, to)`, `cost_within_ceiling(invoke, budget)`,
  `tier_resolved(invoke, tier)`, `dangling_ref_tolerated(ref)`.
