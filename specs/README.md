...
| [`conformance-scenario.md`](conformance-scenario.md) | **KCS** — Conformance-Scenario | test — declarative scenario data that drives **N real** participants over their actual MCP/A2A connections, asserting cross-plane properties, each cited to the clause it checks and decided under §5.1's evidence-precedence rule, plus the optional emitted **exchange record** (§9) | 0.4.0 · 🚧 candidate |
| [`fine-tuning.md`](fine-tuning.md) | **KFT** — Fine-Tuning | profile — composes the four planes into a `finetune` capability | 0.8.0 · 🚧 candidate |
...
- **Status is a lifecycle:** `draft → candidate → ratified`, and the two promotions are gated
  ...
  **All six sit there now** — KINP 0.5.0, KMI 0.3.8, KCB 0.5.8, KCS 0.4.0, KFT 0.8.0 and, since
