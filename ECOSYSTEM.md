...
| [`specs/media-interchange.md`](specs/media-interchange.md) | **KMI** — Media-Interchange | data — media (assets, lineage, OTIO timelines, transforms) | 0.3.8 · 🚧 candidate |
| [`specs/capability-bus.md`](specs/capability-bus.md) | **KCB** — Capability-Bus | control — discover / invoke / subscribe / fetch over MCP + A2A | 0.5.8 · 🚧 candidate |
| [`specs/conformance-scenario.md`](specs/conformance-scenario.md) | **KCS** — Conformance-Scenario | test — declarative scenario data over **N real** participants' own connections, with cross-plane assertions, §5.1's evidence precedence, and the optional emitted **exchange record** (§9) | 0.4.0 · 🚧 candidate |
| [`specs/fine-tuning.md`](specs/fine-tuning.md) | **KFT** — Fine-Tuning | *profile* — composes the four planes into a `finetune` capability (not a fifth plane) | 0.8.0 · 🚧 candidate |
...
| **argos** | Ontolo Labs | producer / consumer | Any-to-any agentic multimedia: KMI assets and lineage in a content-addressed store, analysis claims into KGP, capabilities on KCB. **Narrowed by `D26` (2026-08-19): argos keeps analysis, insights and the neuro-symbolic KB; automated editing moved to `kairos`.** |
| **kairos** | Ontolo Labs | **engine — not a fabric participant** | Headless edit-op engine (deterministic op algebra · CLI contract · render boundary). Driven through its **CLI**, not discovered over KCB — listed here so the topology is complete, not because it plays a protocol role. `argos → kairos` is an optional one-way call: argos may drive it, argos must not require it, and **kairos never dials back**. Takes the `lugh` edit-ops SLM edge. |
...
| **amphora** | personal | **standalone — no protocol role** | Resumable, backgroundable, storage-aware large-file uploader (RN · iOS · Android). Speaks `draft-ietf-httpbis-resumable-upload` (tus 2.x) to tusd, not a koine plane. Listed for completeness; it neither produces nor consumes on any protocol plane. |
